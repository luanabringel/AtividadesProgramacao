...
public class QuickSelect<T extends Comparable<T>> {
	...
	private int partition(T[] array, int leftIndex, int rightIndex){
		...
		int iPivot = leftIndex;
		for (int i = leftIndex; i <= rightIndex; i++){
			if (array[i].compareTo(pivot) < 0){
				util.Util.swap(array, i, iPivot);
				iPivot++;
			}
		}
		util.Util.swap(array, rightIndex, iPivot);
		return iPivot;
	}

	private T quicksort(T[] array, int leftIndex, int rightIndex, int k){
		int partition = partition(array, leftIndex, rightIndex);

		T resultado = null;

		if (partition == k - 1){
			resultado = array[partition];
		} else if (partition < k - 1){
			resultado = quicksort(array, partition + 1, rightIndex, k);
		} else {
			resultado = quicksort(array, leftIndex, partition - 1, k);
		}
		return resultado;
	}
}
