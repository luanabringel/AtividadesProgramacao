	...
	private BSTNode<T> search(BSTNode<T> current, T element) {
		...
		if (element != null) {
			if (current.isEmpty() || current.getData().equals(element))
				...
				busca = search((BSTNode<T>) current.getLeft(), element);
			} else {
				busca = search((BSTNode<T>) current.getRight(), element);
			}
		}
		return busca;
	}
		...
		if (element != null) {
			...
			if (!nodeElement.isEmpty()) {
				if (!nodeElement.getRight().isEmpty()) {
					resultado = this.minimum((BSTNode<T>) nodeElement.getRight());
				} else {
					resultado = sucessor(nodeElement, element);
				}
			}
		}
		return resultado;
		...
		if (element != null) {
			...
			if (!nodeElement.isEmpty()) {
				if (!nodeElement.getLeft().isEmpty()) {
					pre = this.maximum((BSTNode<T>) nodeElement.getLeft());
				} else {
					pre = predecessor(nodeElement, element);
				}
			}
		}
		return pre;
