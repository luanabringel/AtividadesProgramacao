...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		if (position >= 0) {
			int maior = position;
			int esquerda = this.left(position);
			int direita = this.right(position);
			if (esquerda <= this.index && this.comparator.compare(heap[position], heap[esquerda]) < 0) {
				maior = esquerda;
			}
			if (direita <= this.index && this.comparator.compare(heap[maior], heap[direita]) < 0) {
				maior = direita;
			}
			if (maior != position) {
				util.Util.swap(heap, position, maior);
				this.heapify(maior);
			}
		}
	}
	...
	public void insert(T element) {
		...
		if (element != null) {
			...
			while (i > 0 && this.comparator.compare(this.heap[i], this.heap[this.parent(i)]) > 0) {
				util.Util.swap(this.heap, i, this.parent(i));
				i = this.parent(i);
			}
		}
	}
